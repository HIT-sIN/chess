...
var black_pieces =["bP1","bP2","bP3","bP4","bP5","bP6",
        ...
        for(var i = 0 ; i < queens.length;i++){
            if(board[queens[i]] != "x") {
                ...
                while(charIndex < 104){
                    ...
                    if(Object.values(board).includes(id) && id != pos) break;
                    if(id == pos){
                        //console.log("there is check from white queen!!!YAS");
                        return true;
                    }
                }
                // right to downleft
...
function checkInsufficientMaterial() {
    ...
    //console.log(bp.length,wp.length);
...
function canBeTaken(pos){
    ...
    ///////// Knight or Horse
        ...
        if(charIndex + 1  <= 104 && numIndex - 2 >= 1){
            var id = String.fromCharCode(charIndex+1)+(numIndex-2);
            if(id == pos.id){
                //console.log(IsKingInCheck(dummyBoard));
                dummyBoard[piece.id] = id;
                if(pos.children.length != 0)
                    dummyBoard[pos.children[0].id] = "x";
                //console.log(IsKingInCheck(dummyBoard));
     ...
     piece.id == "wP5" || piece.id == "wP6" || piece.id == "wP7" || piece.id == "wP8"){
        ...
        if(pos.children.length != 0){
            ...
            if(numIndex == 2){
                ...
                dummyBoard = Object.assign({}, current_setup);
                //console.log("-- canMoveTo Before: ",current_setup[selected.id]);
                dummyBoard[piece.id] = id2;
                //console.log("-- canMoveTo Before: ",current_setup[selected.id]);
                if($(id).children.length == 0 && $(id2).children.length == 0 && id2 == pos.id && !IsKingInCheck(dummyBoard))
                {
                ...
                //console.log("Check for En Passant");
                ...
                //only move forward.
                dummyBoard = Object.assign({}, current_setup);
                //console.log("Last check for pawn");
                if(id == pos.id) {
                    dummyBoard[piece.id] = id;
                    if(!IsKingInCheck(dummyBoard)){
                        if(numIndex + 1 == 8)
    ...
    if(elem.children.length == 0 && canBeTaken(elem) ){
        ...
        if(blackCastle){
            if(bcLeft){
                ...
                //console.log("left castle true: ",$("d8").children.length);
                removePiece = "O-O-O";
            } else if(bcRight){
                $("f8").appendChild($("bRb"));
...
function movePiece(elem) {
    ...
    if( selected  && selected.nodeType )
    {
        ...
        //console.log("Before: ",current_setup[selected.id])
        if(white_pieces.includes(selected.id,0) && TURN == "white" && canMoveTo(elem)){
            ...
            if(IsKingInCheck(Object.assign({}, current_setup))){
                ...
                //console.log("Black king still in check");
                moveNote = moveNote +"+";
            } else {
                //console.log("Removing king highlight:",prevWKingPos);
            ...
            if(IsKingInCheck(Object.assign({}, current_setup))){
                ...
                //console.log("White king still in check");
                moveNote = moveNote + "+";
            } else {
                //console.log("Removing king highlight:",prevBKingPos);
        ...
        //console.log("After: ",current_setup[selected.id]);
        //console.log(current_setup);
        //console.log(Moved_castle);
...
function promotePawn(selectedPiece){
    ...
    if(selectedPiece.id == "b_R" || selectedPiece.id == "b_B" || selectedPiece.id == "b_K" ||
     ...
     selectedPiece.id == "w_Q") {
        //console.log(promoteWhitePawnTo);
        var id = selectedPiece.id+promoted_counter[selectedPiece.id][0];
        promoted_counter[selectedPiece.id][0]++;
        var newPiece = selectedPiece.cloneNode();
